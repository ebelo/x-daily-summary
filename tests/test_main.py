...
@patch("fetchers.mastodon_fetcher.MastodonFetcher.fetch_posts")
@patch("fetchers.mastodon_fetcher.MastodonFetcher.is_configured")
@patch("fetchers.bluesky_fetcher.BlueskyFetcher.fetch_posts")
@patch("fetchers.bluesky_fetcher.BlueskyFetcher.is_configured")
@patch("fetchers.x_fetcher.XFetcher.fetch_posts")
@patch("fetchers.x_fetcher.XFetcher.is_configured")
@patch("main._load_env")
@patch("main.Path.exists")
@patch("main.Path.write_text")
def test_main_fetch_all_sources(
    mock_write_text, mock_exists, mock_load_env,
    mock_x_configured, mock_x_fetch,
    mock_bsky_configured, mock_bsky_fetch,
    mock_mastodon_configured, mock_mastodon_fetch,
    mock_build, mock_generate, capsys
):
    """Test full main() flow using API fetches — verifies fetcher loop."""
    mock_exists.return_value = True
    mock_x_configured.return_value = True
    mock_bsky_configured.return_value = True
    mock_mastodon_configured.return_value = True

    mock_x_fetch.return_value = [{"text": "Hello", "platform": "x", "author_username": "user", "engagement_score": 10}]
    mock_bsky_fetch.return_value = [{"text": "Hello Bsky", "platform": "bluesky", "author_username": "user", "engagement_score": 5}]
    mock_mastodon_fetch.return_value = [{"text": "Hello Mastodon", "platform": "mastodon", "author_username": "user", "engagement_score": 2}]
    ...
    mock_x_fetch.assert_called_once()
    mock_bsky_fetch.assert_called_once()
    mock_mastodon_fetch.assert_called_once()
...
@patch("main.Path.exists")
@patch("main.Path.write_text")
def test_main_fetch_bluesky_only(
    mock_write_text, mock_exists, mock_load_env,
    mock_x_configured,
    mock_bsky_configured, mock_bsky_fetch,
    mock_build, mock_generate, capsys
):
    """Test main() flow with --source bluesky — X fetcher should NOT be called."""
    mock_exists.return_value = True
    mock_x_configured.return_value = True  # X IS configured but source filter excluded it
    mock_bsky_configured.return_value = True
    mock_bsky_fetch.return_value = [{"text": "Hello Bsky", "platform": "bluesky", "author_username": "user", "engagement_score": 5}]
