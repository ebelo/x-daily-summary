...
- **PowerShell Command Chaining**: *Warning:* Windows PowerShell 5.1 (default) does not support the `&&` or `||` operators. When chaining commands in a single `run_command` call, always use the semicolon (`;`) as a separator (e.g., `git commit -am "msg"; git push origin main`). Only use `&&` if the environment is strictly confirmed to be PowerShell 7+ or Bash.
